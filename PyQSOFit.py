...
def Fe_flux_balmer(xval,pp):
    ...
    Fe_FWHM = pp[1]
    # What is this pp[1]? (It is a fwhm component) What is the width of the Fe tempelate being used.Fwhm_feorg  fwhm_con=sqrt(fwhm_hbeta**2-fwhm_feorg**2)
    xval_new = xval*(1.0 + pp[2])
...
# Flux is converted to luminosity using this simple formulation.
def flux2L(flux,z):
...
class QSOFit():
    ...
    def Fit(self, name = None, nsmooth = 1,and_or_mask = True,reject_badpix = True, deredden = True,wave_range = None,\
        ...
        # do host decomposition (The reason for this limit is that the tempelates are not available beyond that).--------------
    ...
    def _HostDecompose(self,wave,flux,err,z,Mi,npca_gal,npca_qso,path):
        ...
        if self.BC03 == False:
            galaxy = fits.open('pca/Yip_pca_templates/gal_eigenspec_Yip2004.fits')
            gal = galaxy[1].data
        ...
        if Mi is None:
            quasar = fits.open('pca/Yip_pca_templates/qso_eigenspec_Yip2004_global.fits')
        else:
            if -24 < Mi <= -22 and 0.08 <= z < 0.53:
                quasar=fits.open('pca/Yip_pca_templates/qso_eigenspec_Yip2004_CZBIN1.fits')
            elif -26 < Mi <= -24 and 0.08 <= z < 0.53:
                quasar=fits.open('pca/Yip_pca_templates/qso_eigenspec_Yip2004_DZBIN1.fits')
            elif -24 < Mi <= -22 and 0.53 <= z < 1.16:
                quasar=fits.open('pca/Yip_pca_templates/qso_eigenspec_Yip2004_BZBIN2.fits')
            elif -26 < Mi <= -24 and 0.53 <= z < 1.16:
                quasar=fits.open('pca/Yip_pca_templates/qso_eigenspec_Yip2004_CZBIN2.fits')
            elif -28 < Mi <= -26 and 0.53 <= z < 1.16:
                quasar=fits.open('pca/Yip_pca_templates/qso_eigenspec_Yip2004_DZBIN2.fits')
            else:
    ...
    def _DoContiFit(self,wave,flux,err,ra,dec,plateid,mjd,fiberid):
        ...
        if self.initial_guess is not None:
            ...
            pp0 = np.array([0., 3000., 0., 0., 3000., 0., 1., -1.5, 0., 15000., 0.5, 0., 0., 0.]) # tie up this fwhm with Hb_br component[1] after converting it in km/sec. fix it.
        ...
        tmp_parinfo = [{'limits':(0.,10.**10)},{'limits':(1200.,10000.)},{'limits':(-0.01,0.01)},\
                       ...
                       {'limits':(0.,10.**10)},{'limits':(10000.,50000.)},{'limits':(0.1,2.)},\
                        None,None,None] #How to fix the first index parameter.
    ...
    def _f_conti_all(self, xval, pp):
        ...
        Continuum components described by 14 parameters
         ...
         pp[6]: norm_factor for continu[um f_lambda = (lambda/3000.0)^{-alpha}
    ...
    def _DoLineFit(self,wave,line_flux,err,f):
        ...
        #read line parameter
        linepara = fits.open('qsopar.fits')
    ...
    def line_prop(self,compcenter,pp,linetype):
        ...
        if n_gauss == 0:
            ...
            #print cen,sig,area (what is happening here?)
            left = min(cen-3*sig)
            right = max(cen+3*sig)
            disp = 1.e-4*np.log(10)*0.01
    ...
    def _SaveResult(self, conti_result, conti_result_name, line_result, line_result_name, save_fits_path, save_fits_name):
        ...
        t.write(save_fits_name+'.fits', format = 'fits', overwrite = True)
    ...
    def _PlotFig(self, ra, dec, z, wave, flux, err, decomposition_host, linefit, tmp_all, gauss_result, f_conti_model, conti_fit,\
        ...
        plt.plot(self.wave_prereduced,self.flux_prereduced,color='black',alpha=0.5,label = 'Observed spectrum')
        ...
        if linefit == True:
            if self.MC == True:
                ...
                plt.plot(wave,manygauss(np.log(wave),gauss_result[::2])+f_conti_model,'k', label = 'Emission line', lw = 1)
            else:
                ...
                plt.plot(wave,manygauss(np.log(wave),gauss_result)+f_conti_model,'k', label = 'emission line', lw = 1)
        plt.plot([0,0],[0,0],'r', label = 'broad component')
        plt.plot([0,0],[0,0],'g', label = 'narrow component')
        plt.plot(wave,f_conti_model,'c',lw=1, label = 'FeII')
        if self.BC == True:
            plt.plot(wave,self.f_pl_model+self.f_poly_model+self.f_bc_model,'y',lw = 1, label = 'BC')
        plt.plot(wave,conti_fit.params[6]*(wave/3000.0)**conti_fit.params[7]+f_poly_conti(wave, conti_fit.params[11:]), color = 'orange', lw = 1, label = 'conti')
        ...
        if self.plot_line_name == True:
            ...
            line_name = np.array(['',  '', r'$H\alpha+NII$', '', 'SII6718,6732', r'$H\beta$', '[OIII]',\
                    'HeII4687',r'$H\gamma$','CaII3934', 'OII3728', 'NeV3426', 'MgII','CIII]',\
        ...
        if linefit == True:
            ...
            for c in range(self.ncomp):
                ...
                plt.plot(wave,self.line_flux,color='black',lw=1)
                ...
                if self.MC == True:
                    for p in range(int(len(gauss_result)/6)):
                        if self.CalFWHM(gauss_result[2+p*6],gauss_result[4+p*6] ) < 1200.:
                            ...
                            color = 'blue'
                        plt.plot(wave,manygauss(np.log(wave),gauss_result[::2][p*3:(p+1)*3]), color = color,lw=1)
                    plt.plot(wave,manygauss(np.log(wave),gauss_result[::2]),'r',lw=1)
                else:
                    for p in range(int(len(gauss_result)/3)):
                        if self.CalFWHM(gauss_result[3*p+1],gauss_result[3*p+2] ) < 1200.:
                            color = 'green'
                            ...
                            color = 'blue'
                        plt.plot(wave,onegauss(np.log(wave),gauss_result[p*3:(p+1)*3]), color = color,lw=1)
                    plt.plot(wave,manygauss(np.log(wave),gauss_result),color='red',lw=1)
                    kk=self.line_flux-(manygauss(np.log(wave),gauss_result))
                #plt.plot(wave,kk,'k')
